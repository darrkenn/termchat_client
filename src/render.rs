...
pub fn render(frame: &mut Frame, app: &mut App) {
    let area = frame.area();
    let mut messages = app.messages.lock().unwrap().clone();
    let mut list_state = app.list_state;
    let msg_buffer = &app.msg_buffer;
    ...
    let max_visable = visible_height as usize;

    if messages.len() > max_visable {
        let limit = messages.len() - max_visable;
        messages.drain(0..limit);
    }
    ...
    render_message_list(frame, message_list, vertical_chunks[0], &mut list_state);
